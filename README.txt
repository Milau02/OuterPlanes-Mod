Ported:
  - Garnet Gem / Garnet Cluster
  - Garnet Ore

  - Soultree Logs / Wood (With stripped variants)
  - Soultree Planks
  - Soultree Slab
  - Soultree Fence / Fencegate
  - Soultree Stairs
  - Soultree Door
  - Soultree Trapdoor
  - Soultree Trees / Saplings / Leaves / Roots

  - Astral Logs / Wood (With stripped variants)
  - Astral Planks
  - Astral Slab
  - Astral Fence / Fencegate
  - Astral Stairs
  - Astral Door
  - Astral Trapdoor
  - Astral Trees / Saplings / Leaves / Roots

To Port:
  - Soulstone
  - Soul Gate (Reworking entirely, separating block functionality into an Astral minecraft effect to handle the player's transformation)

